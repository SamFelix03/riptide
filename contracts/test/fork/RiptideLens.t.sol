...
contract RiptideLensTest is RiptideForkBase {
    ...
    function test_lensLiveAquaAfterPush() public {
        ...
        IRiptideLens.StrategyState memory beforeState =
            lens.strategyState(maker, orderHash, address(tokenBase), address(tokenQuote));
        ...
        IRiptideLens.StrategyState memory afterPush =
            lens.strategyState(maker, orderHash, address(tokenBase), address(tokenQuote));

        assertEq(afterPush.aquaQuote, beforeState.aquaQuote + 25_000e18);
        assertEq(afterPush.aquaBase, beforeState.aquaBase);
    }
}
