"""Generate or verify committed JSON test vectors."""
...
from .vector_cases import all_vector_files
...
def write_vectors() -> None:
    ...
    for name, content in _serialize_payload(all_vector_files()).items():
        (VECTORS_DIR / name).write_text(content, encoding="utf-8", newline="\n")


def check_vectors() -> int:
    files = _serialize_payload(all_vector_files())
    exit_code = 0
